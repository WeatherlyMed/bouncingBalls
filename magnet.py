...
line_thickness = 10
max_slope = 0.05  # Maximum tilt slope (5%)
...
gravity = 0.5
magnet_strength = 1.0  # Increased magnetic acceleration strength
...
def draw_tilting_line(time_elapsed):
    ...
    return (line_start_x, line_start_y, line_end_x, line_end_y, slope)
...
def update_ball(line_coords):
    ...
    (line_start_x, line_start_y, line_end_x, line_end_y, slope) = line_coords
    # Line equation: y = slope * x + intercept
    intercept = line_start_y - slope * line_start_x
    
    # Calculate the y position of the line at the ball's x position
    line_y_at_ball_x = slope * ball_x + intercept
    
    if ball_y + ball_radius > line_y_at_ball_x:
        # Move ball above the line if it's below
        ball_y = line_y_at_ball_x - ball_radius
        # Reflect ball velocity off the line
        normal = (-slope, 1)
